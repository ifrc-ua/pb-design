...
Палітра тримається на двох кольорах з логотипу: **фіолетовий `#654EA3`** (серйозність, влада, креативність) і **жовтий `#FFEC08`** (енергія, голос, видимість). Фіолетовий як чорнило документа, жовтий як маркер для найважливішого. Фон майже білий (`#FDFDFD`), текст глибокий графітовий (`#1A1A1A`), що є парою для максимальної читабельності і фокусу на даних. Ненасичена дисципліна кольору — у ядрі тримаємо тільки два бренд-кольори + сірі нейтрали; розширена data-viz-палітра (коли треба передати 5+ категорій на одному графіку) живе в окремому `design-data.md`, що створюється за потреби.
...
- Акуратні тіні, 8px сітка, широкі міжколонки — муніципальна довіра = «не тісно»
...
| Роль | Шрифт | Розмір | Насиченість | Висота рядка | Міжлітерний | Примітки |
|------|------|------|--------|-------------|----------------|-------|
| Герой / Дисплей | Phenomena | 72–96px | 900 (Black) | 1.00 | -1.5px до -2px | Числа-герої, заголовок лендингу, «10 років БУ» |
| H1 / Заголовок сторінки | Phenomena | 48–64px | 700 (Bold) | 1.05 | -0.8px | Головний заголовок екрану |
| H2 / Секція | Phenomena | 32–40px | 700 (Bold) | 1.15 | -0.4px | Секції сторінки |
| H3 / Підсекція | Phenomena | 24–28px | 400 (Regular) | 1.25 | -0.2px | Підзаголовок секції |
| Назва блоку | Proxima Nova | 20–24px | 600 (Semibold) | 1.30 | normal | Назва картки проєкту, заголовок віджета |
| Текст великий | Proxima Nova | 18–20px | 400 (Regular) | 1.55 | normal | Вступні абзаци, описи |
| Текст стандарт | Proxima Nova | 16px | 400 (Regular) | 1.55 | normal | Основний текст |
| Текст малий | Proxima Nova | 14px | 400 (Regular) | 1.50 | normal | Допоміжний текст, метадані |
| Підпис | Proxima Nova | 12–13px | 500 (Medium) | 1.40 | normal | Підписи під графіками/зображеннями |
| Надрядок / Ярлик | Proxima Nova | 11–12px | 600 (Semibold) | 1.20 | +1px (uppercase) | Ярлики секцій, теги |
| Статистика (велике число) | Phenomena | 48–72px | 900 (Black) | 1.00 | -1px, tabular-nums | «14 832 голоси», «₴5.2 млн» |
| Підпис даних | Proxima Nova | 11–13px | 400 (Regular) | 1.30 | normal, tabular-nums | Осі графіків, значення підказок |
...
- **Муніципальна довіра = не тісно.** Секції розділяються 80–96px вертикального простору на десктопі (48–64px мобільний).
- **Картки дихають:** відступ усередині картки 20–24px, між картками у сітці — 24px.
