...
- Data-viz у ядрі — Primary shades + жовтий; розширена палітра для аналітики виноситься в `design-data.md` (створюється за потреби)
...
> Розширена категорійна палітра (7+ різних кольорів для donut або stacked-bar з багатьма сегментами) й повна мапа «архетип категорії → колір» на основі **реальних** категорій БУ 2016–2026 (Освіта, Благоустрій, Малі вулиці, Зелень, Культурна спадщина, Допомога ЗСУ, Доступність) — у майбутньому `design-data.md`. Створюватиметься при першому реальному аналітичному артефакті, а не наперед.
...
> Фіксована таблиця «категорія → іконка → колір» і специфіка іконок для мап (маркери проєктів, кластери, зірочка переможця) — у майбутньому `design-data.md`. Тут тримаємо тільки загальні правила (товщина, розміри, `currentColor`), бо категорії БУ змінювалися рік-у-рік і прив'язувати іконки до конкретного набору в ядрі — створювати вигадку, яка не відповідає реальним даним 2016–2026.
...
**Відкладено в `design-data.md`** (створюється коли першому реальному аналітичному артефакту це буде потрібно):
- Розширена категорійна палітра (7+ кольорів для stacked-графіків, donut'ів з багатьма сегментами)
- Мапа «категорія → колір» на основі реальних категорій БУ 2016–2026 (Освіта, Благоустрій, Малі вулиці, Зелень, Культурна спадщина, Допомога ЗСУ, Доступність)
- Мапа «категорія → іконка»
- Повний набір 7 реальних статусів (Реалізований, На реалізації, Брав участь, Відхилений, Відхилений остаточно, Неможливо реалізувати, Видалений) — у ядрі лише 2 промо-бейджі (Переможець, Реалізовано)
- Специфіка мап: маркери проєктів, кластери, зірочки переможців, шкали choropleth
...
**Якщо агент натикається на прогалину**: залиште в згенерованому артефакті HTML-коментар `<!-- TODO: design-data.md потребує X -->`, ніколи не вигадуйте токен чи колір поза задокументованим набором.
