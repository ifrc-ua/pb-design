...
- Номер слайду + «БУ ІФ · 10 років» у нижньому лівому куті, Proxima Nova 14px 500 `#6A6C77`
...
- Overline "ГРОМАДСЬКА ПРЕЗЕНТАЦІЯ · 2026" in Proxima Nova 20px 600 uppercase letter-spacing +2px color #6A6C77 at y=320
...
Bottom-right: small URL "pb.if.ua" in Proxima Nova 18px 500 color #6A6C77.
```
...
Background #FDFDFD.
Content:
- Eyebrow line at y=280: "У 2021–2026" in Proxima Nova 28px 600 uppercase letter-spacing +2px color #654EA3, x=160
- Giant number centered: "110 000" in Phenomena Black 480px color #1A1A1A tabular-nums, center-aligned horizontally, y=450 baseline
- Below number: "унікальних виборців" in Phenomena 64px 400 color #1A1A1A, centered
- Footnote annotation: "понад 110 тисяч людей проголосували щонайменше раз; поіменний облік ведеться з 2021 року" Proxima Nova 24px 500 italic color #6A6C77, centered, y=bottom-120
...
- Chart 720×480: grouped bars per year, primary bars #654EA3, highlighted year 2020 #FFEC08 with dark stroke
...
- Supporting paragraph below: "У 2020 бюджет стрибнув утричі — з 5 до 15 мільйонів, а кількість подань зросла з 130 до 303. Мешканці хотіли змінити простір навколо себе." Proxima Nova 22px 400 color #3F4049 line-height 1.55, max-width 720px
- Key stat pull-quote bottom: "×3" in Phenomena Black 120px color #654EA3 tabular-nums, label "зростання бюджету у 2020" Proxima Nova 18px 500 uppercase letter-spacing +1px color #6A6C77
...
Title overlay on map top-right: "Активність 2016–2026 · села — з 2020" small pill Proxima Nova 14px 600 uppercase on white #FDFDFD semi-opaque bg. (До 2020 конкурс охоплював лише місто — хороплет за весь період показує села тільки за 2020+.)
```
...
- Below: 3 stat rows, each:
  - Label Proxima Nova 18px 600 uppercase letter-spacing +1px color #6A6C77
...
- Centered text "За 10 років бюджет зріс у 110 разів, голоси — у 108" Phenomena 32px 700 color #1A1A1A
```
...
- Year labels below each dot: Proxima Nova 16px 600 tabular-nums color #6A6C77
...
- Examples: "2016: старт із пів мільйона", "2019: реформа правил і Школа БУ", "2020: громада росте до 19 населених пунктів", "2023: конкурс повертається після паузи 2022", "2026: рекорд — 226,6 тис. голосів"

Footer: "Повний таймлайн у додатку" Proxima Nova 18px 500 color #6A6C77, bottom-right.
```
...
- 4px gap
- Project meta: "{Двір на Галицькій · 2 847 голосів · реалізовано 2020}" Proxima Nova 18px 500 color #6A6C77
```
...
Example takeaways (реальні висновки зі статті «10 років БУ»):
1. 🏫 "Школи виграють кількістю, армія — грошима" / "Освіта бере понад половину голосів, але п'яту частину грошей; армія — 9% голосів і майже 40% коштів (2025–2026)"
2. 🌾 "Малі села активніші за місто" / "Добровляни — 14,4 проєкта на тисячу дорослих проти 7,4 у місті"
3. ⏱ "Голосують на старті, не в останню ніч" / "Перші три дні кампанії дають до 40% голосів, останні три — лише 9–18%"

Bottom full-width CTA band:
- y=900, height 180px, bg #1A1A1A
- Left: "Дослідити 781 тисячу голосів →" Phenomena 36px 700 color #FDFDFD
...
- Contact line: "ім'я.прізвище@... · @ifbudget" Proxima Nova 24px 500 color #6A6C77
...
4. Big number: "1 565 проєктів" — use the `Big Number slide` template
...
8. Big number: "110 000 виборців" — `Big Number slide` template variant
